...
//				.filter(this::predAdjFilter)
				.map(parsedRecord -> parsedRecord.printRecord())
				.parallel()
	...
	private static class ParsedRecord {
		...
		public String printRecord() {
			...
			return record.printRecord() + ':' + relationsCollector.printShortFormula() + ':' + relationsCollector.printFormula() + ':' + relationsCollector.printRelations();
		}
	}
	...
	private static String featureTreeToString(FeatureNode featureNode, String alignment, Visited visited) {
		...
		return "<" + id + ">: \n" + alignment + featureNode.getFeatureNames().stream()
				.map(feature -> feature + ": " + featureTreeToString(featureNode.get(feature), alignment + " ", visited))
