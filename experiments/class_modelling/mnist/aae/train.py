...
import random
import os
from mnist import mnist_n, next_batch
...
def main(inliner_class, outliner_class): # folding_id, folds=5
    batch_size = 128
    zsize = 32
    mnist_train = []
    #mnist_valid = []
    ...
    '''
    mnist_train, train_size = mnist_n(inliner_class, True)
    #mnist_valid, valid_size = mnist_n(outliner_class, True)
    ...
    for epoch in range(train_epoch):
        ...
        for it in range(train_size // batch_size):
            #x = extract_batch(mnist_train_x, it, batch_size).view(-1, 1, 32, 32)
            x = setup(next_batch(mnist_train, batch_size))
            #print(x.shape)
            ...
            z = torch.randn((x.shape[0], zsize)).view(-1, zsize, 1, 1) # batch_size
            z = Variable(z)
        ...
        Gtrain_loss /= (train_size)
        Dtrain_loss /= (train_size)
        ZDtrain_loss /= (train_size)
        GEtrain_loss /= (train_size)
        Etrain_loss /= (train_size)
        ...
        with torch.no_grad():
            ...
            save_image(resultsample.view(64, 1, 32, 32), 'results'+str(inliner_class)+'/sample_' + str(epoch) + '.png')
...
if __name__ == '__main__':
    #main(0, [0], 10)
    # main(folding_id, inliner_classes, total_classes, folds=5)
    main(0, 5)
