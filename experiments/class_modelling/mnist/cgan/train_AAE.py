...
def main(folding_id, inliner_classes, total_classes, folds=5, zsize=None):
    batch_size = 128
    #zsize = 32
    mnist_train = []
    mnist_valid = []
    ...
    G_optimizer = optim.Adam(list(E.parameters()) + list(G.parameters()), lr=lr, betas=(0.5, 0.999))
    ...
    for epoch in range(train_epoch):
        ...
        for it in range(len(mnist_train_x) // batch_size):
            ...
            D_result = D(
                    x,
                    torch.clamp(torch.randn(x.shape) + x, min=0.0, max=1.0)
            ).squeeze()
            ...
            D_result = D(
                    x,
                    torch.clamp(torch.randn(x_fake.shape) + x_fake, min=0.0, max=1.0)
            ).squeeze()
            ...
            E_loss = BCE_loss(ZD_result, y_real_z)*0 # 0.5

            Recon_loss = F.binary_cross_entropy(x_d, x)*0.15 # 1.0
    ...
    torch.save(G.state_dict(), f"Gmodel{inliner_classes[0]}_z{zsize}.pkl")
    torch.save(E.state_dict(), f"Emodel{inliner_classes[0]}_z{zsize}.pkl")
    #torch.save(D.state_dict(), f"Dmodel.pkl")
    #torch.save(ZD.state_dict(), f"ZDmodel.pkl")
