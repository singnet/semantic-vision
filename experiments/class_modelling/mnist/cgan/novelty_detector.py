...
def main(folding_id, inliner_classes, total_classes, folds=5, z_size=None):
    batch_size = 64
    mnist_train = []
    mnist_valid = []
    #z_size = 32
    ...
    G.load_state_dict(torch.load(f"Gmodel{inliner_classes[0]}_z{z_size}.pkl"))
    E.load_state_dict(torch.load(f"Emodel{inliner_classes[0]}_z{z_size}.pkl"))
    ...
    def compute_threshold(mnist_valid, percentage):
        ...
        for it in range(len(mnist_valid_x) // batch_size):
            ...
            for i in range(batch_size):
                ...
                logPz = np.sum(np.log(p)) # p[np.nonzero(p)]
...
if __name__ == '__main__':
    ...
    main(0, [args.n], 10, z_size=args.z)
