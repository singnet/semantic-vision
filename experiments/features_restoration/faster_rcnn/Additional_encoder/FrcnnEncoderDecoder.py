...
def d(z):
    ...
    d1 = tf.layers.dense(z, 4 * 4 * 128, name='d_dense1', reuse=d.reuse)
    a1 = tf.nn.relu(d1)
...
bounding_boxes = tf.placeholder(tf.float32, [None, 128, 128, 3], name="bounding_boxes")
rcnn_features = tf.placeholder(tf.float32, [None, 2048], name="rcnn_features")

encoder_features = e(bounding_boxes)
d_input = tf.concat((rcnn_features, encoder_features), axis=1)
