...
def spread_loss(output, pose_out, x, y, m):
    ...
    if cfg.weight_reg:
        ...
        regularization = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
        # loss+0.0005*reconstruction_loss+regularization#
        loss_all = tf.add_n([loss] + [0.05 * data_size* data_size * reconstruction_loss] + regularization)
    else:
        loss_all = tf.add_n([loss] + [0.05 * data_size* data_size * reconstruction_loss])
...
def test_accuracy_excl(logits, labels):

    v3 = tf.fill([1,cfg.batch_size], 3)
    v4 = tf.fill([1,cfg.batch_size], 4)
    logits_idx = tf.to_int32(tf.argmax(logits, axis=1))

    # labels_idx = tf.reshape(labels_idx, shape=(cfg.batch_size,))
    mask_v3 = tf.equal(labels, v3)
    mask_v4 = tf.equal(labels, v4)

    mask = tf.logical_or(mask_v3, mask_v4)
    mask = tf.squeeze(tf.transpose(mask))

    labels_excl = tf.boolean_mask(labels, mask)
    logits_excl = tf.boolean_mask(logits_idx, mask)
    logits_excl = tf.cast(logits_excl, dtype=tf.int32)

    correct_preds = tf.equal(tf.to_int32(labels_excl), logits_excl)
    mask = tf.cast(mask, dtype=tf.int32)

    s = tf.reduce_sum( mask )
    s = tf.cast(s, dtype=tf.float32)
    rs = tf.reduce_sum(tf.cast(correct_preds, tf.float32))

    accuracy = tf.stack([rs, s], axis=0)

    return accuracy

def test_accuracy_sum(logits, labels):
...
def test_predict(logits, labels):
