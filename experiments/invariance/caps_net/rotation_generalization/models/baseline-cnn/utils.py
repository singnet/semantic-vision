...
def get_random_mnist_batch(x, y, batch_size):
    ...
    for k in range(batch_size):
        ...
        xi = x[i, :, :, :]
        yi = y[i, :]
        batch_x[k, :, :, :] = create_inputs_mnist_rot_excl(xi, yi)
        batch_y[k, :] = y[i, :]
...
def save_img_batch(batch, filename):
    # x = batch*127.5 + 127.5
    x = batch * 255
    img = combine_images(x)
    Image.fromarray(img.astype(np.uint8)).save(filename)

def rotate_batch_for_test(x, y, ang_excl_range):
    batch_size = x.shape[0]
    batch_x = np.zeros(x.shape)
    ang1 = ang_excl_range[0]
    ang2 = ang_excl_range[1]
    for i in range(batch_size):
        yi = y[i,:]
        yi = np.argmax(yi)
        if (yi == 3) or (yi == 4):
            a = np.random.uniform(ang1, ang2)
            xi = rotate_img(x[i, :, :, :], a)
        else:
            a = np.random.uniform(0, 360)
            xi = rotate_img(x[i, :, :, :], a)
