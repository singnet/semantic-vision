...
IMG_DIM    = (28, 28, 1)
Z_DIM      = 80
ZBC_DIM    = 3 # Number of "binary categorical units"
ZU_DIM     = 0
...
def q_tf(x, reuse = True):
    with tf.variable_scope("Q", reuse = reuse):
        ...
        x = tf.contrib.layers.flatten(x)
        x = tf.layers.dense(x, 1024, activation=lrelu)        
        x = fully_connected(x, ZBC_DIM + ZU_DIM , activation_fn=None)
        x = tf.identity(x, name="output")
        return x    
...
def q_cost_tf(z, q):
    ...
    z_uni = z[:, ZBC_DIM: ZBC_DIM + ZU_DIM]
    q_uni = q[:, ZBC_DIM: ZBC_DIM + ZU_DIM]
