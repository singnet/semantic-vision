...
#    e_train_op = tf.train.RMSPropOptimizer(learning_rate=LEARNING_RATE).minimize(e_cost, var_list=e_param)
    ...
    for it in range(ITERS):        
        ...
        shifts, Xb, Xb_rot = batch.get_batch_partshift(X_train, Y_train, BATCH_SIZE, MAX_SHIFT, PART_SHIFT)        
        ...
        if ((it + 1) % 500 == 0):
            ...
            shifts, Xb, Xb_rot = batch.get_batch_partshift(X_test, Y_test, BATCH_SIZE, MAX_SHIFT, PART_SHIFT)
            samples = sess.run([rec_data], feed_dict={real_data: Xb, real_data_rot: Xb_rot, shifts_tf : shifts})
