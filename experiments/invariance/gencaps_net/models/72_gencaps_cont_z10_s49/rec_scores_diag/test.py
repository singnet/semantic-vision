...
def simple_batch_run(sess, in_tf, out_tf, in_np, batch_size):
    
    out_np_all = []
    for in_batch_np in np.array_split(in_np, len(in_np)//batch_size + 1):
        out_np_batch  = sess.run(out_tf, feed_dict={in_tf: in_batch_np})
...
def print_for_model_digit(model, tag):
    ...
    for digit in range(10):
        ...
        BATCH_SIZE = len(data)
        z = simple_batch_run(sess, input_enc, output_enc, data, BATCH_SIZE)
        ...
        for s in np.arange(-20,20,2):
            ...
            samples = simple_batch_run(sess, input_rec, output_rec, za, BATCH_SIZE)
