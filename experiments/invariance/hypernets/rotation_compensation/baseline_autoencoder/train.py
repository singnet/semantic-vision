...
def reconstructor_tf(x, reuse = True):
...
def train():
    ...
    rec_data      = reconstructor_tf(encoded_data, reuse = False) 
    rec_real_z    = reconstructor_tf(real_z, reuse = True)
    ...
    for it in range(ITERS):        
        ...
        angles, Xb_rot, Xb = get_batch(X_train, Y_train)        
        ...
        if ((it + 1) % 500 == 0):
            ...
            samples = sess.run([rec_real_z], feed_dict={real_z: fix_z})
            ...
            r_cost_rez, d_cost_rez, e_cost_rez = sess.run([r_cost, d_cost, e_cost], 
                                                          feed_dict={real_data: Xb, real_data_rot: Xb_rot, real_z: random_z()})
