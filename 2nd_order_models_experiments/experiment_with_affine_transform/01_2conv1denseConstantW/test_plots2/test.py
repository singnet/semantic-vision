...
def plot_for_model(model, tag):
    ...
    for digit in range(10):
        ...
        csamples = []
        creal    = []
        for affineParam in np.arange(-1,1,0.1):
            affineParamArray = np.ones(BATCH_SIZE) * affineParam
            affineParamArrayx2 = np.stack(arrays=[affineParamArray, affineParamArray], axis=-1)
            affineParamArrayx7 = np.stack(arrays=[affineParamArray, affineParamArray, affineParamArray, affineParamArray, affineParamArray, affineParamArray, affineParamArray], axis=-1)
            Xb_rot, output = rotatepair_batch.AffineTransform(Xb, affineParamArrayx2, affineParamArray, affineParamArrayx2, affineParamArrayx2)
            if PARAM_SIZE == 7:
                samples = sess.run(output_img, feed_dict={input_img: Xb, input_parameters: affineParamArrayx7})
            else:
                samples = sess.run(output_img, feed_dict={input_img: Xb, input_parameters: output})
